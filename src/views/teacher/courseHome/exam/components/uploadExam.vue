<template>
  <div class="uploadExamBox">
    <el-tabs
      ...
      class="demo-tabs"
    >
      <el-tab-pane label="上传原试卷">
        <el-select
          v-model="originPaperType"
        ...
        <el-upload
          v-if="originPaperType === 0"
          action="http://localhost:8079/activity/getImagePath"
          method="post"
          name="image"
        ...
        <el-dialog width="80%" height="600px" v-model="originDialog">
          <img
            ...
            :src="originDialogImageUrl"
            alt=""
          />
        </el-dialog>

        <el-upload
          v-if="originPaperType === 1"
          ref="originUpload"
          class="upload-demo"
          :limit="1"
          :auto-upload="false"
          style="margin-left: 20px"
          :on-change="originSubmit"
          accept=".doc,.docx"
        >
          <template #trigger>
        ...
        >
          <template #extra>
            <el-button type="primary" @click="toViewOrigin"
              >识别成功，去编辑识别结果</el-button
            >
          </template>
        </el-result>

        <el-result
          v-else-if="originFlag === 0"
          icon="error"
          title="Error Tip"
          sub-title="Please follow the instructions"
        >
          <template #extra>
            <el-button type="primary" @click="originFlag = -1"
              >识别失败，请再次尝试</el-button
            >
          </template>
        </el-result>
      </el-tab-pane>

      <el-tab-pane label="上传试卷答案">
        <el-upload
          ref="upload"
          class="upload-demo"
          :limit="1"
        ...
        <!-- <span class="titleText">识别结果</span> -->
        ...
        >
          <template #extra>
            <el-button type="primary" @click="toViewAnswer"
              >识别成功，去编辑识别结果</el-button
            >
          </template>
        </el-result>
        ...
        <el-result
          v-else-if="answerFlag === 0"
          icon="error"
          title="识别失败"
          sub-title="识别失败，(；′⌒`)"
        >
          <template #extra>
            <el-button type="primary" @click="answerFlag = -1"
              >识别失败，请再次尝试</el-button
            >
          </template>
        </el-result>
      ...
      <el-tab-pane label="批量上传学生试卷">
        ...
        >
          <template #extra>
            <el-button type="primary" @click="toViewStudent"
              >识别成功，去编辑识别结果</el-button
            >
          </template>
        </el-result>
...
const toViewOrigin = () => {
  // router.push("/upload/origin/" + route.params.paperId);
  window.open("http://localhost:5173/upload/origin/" + route.params.paperId);
};
...
const toViewAnswer = () => {
  window.open("http://localhost:5173/upload/answer/" + route.params.paperId);
};
...
const toViewStudent = () => {
  window.open("http://localhost:5173/upload/student/" + route.params.paperId);
};

const toViewResult = () => {
  window.open("http://localhost:5173/upload/result/" + route.params.paperId);
};
