      ...
      <div v-else-if="choose === '简答题'" class="reply" style="margin-bottom: 20px">
        ...
        <div class="replyOptions" style="margin-top: 20px">
          <div style="" class="editor">
            <myEditor :text="replyText" ref="replyAnswer"></myEditor>
          </div>
        </div>
      </div>
...
import {
  teacherAddQuestionAPI,
  teacherAddSingleQuestionAPI,
  teacherGetQuestionDetailsAPI,
  teacherModifyQuestionAPI,
} from "@/apis/question";
...
let questionData = {
  id: parseInt(route.params.questionId as string),
  type: "",
  title: "",
  answer: "",
  answerAnalysis: "",
...
const addNewQuestion = async () => {
  ...
  // 发送请求
  const res = await teacherModifyQuestionAPI(questionData);
...
const getQuestionDetails = async () => {
  const res = await teacherGetQuestionDetailsAPI(
    parseInt(route.params.questionId as string)
  );
  ...
  if (res.data.code === 200) {
    ...
    if (res.data.data.type === "单选题") {
      ...
      fillAnswer.value = JSON.parse(JSON.parse(res.data.data.title).options);
    } else if (res.data.data.type === "判断题") {
